...
先处理 `BUG-012` / `BUG-016` 与 `BUG-013`。其中 `BUG-013` 已由 2026-06-19 13:43 Chrome 真实浏览器复测确认通过；后续如果改动导入链路仍要回归。当前最大阻断仍是 UI 高保真还原：
...
2026-06-19 13:43 最新回归补充：

- `BUG-016`：真实 Chrome 登录态完整跑通 Step1→Step5 后，UI 仍明显不像 Figma。左侧侧栏仍是约 52px 窄图标栏，不是 Figma 展开态 288px 文本导航；主内容集中在左侧，右侧大片空白；STEP5 没有形成 Figma 的左能力切换 + 中市集卡 + 右字段来源三栏。修复时必须优先对照本轮截图目录 `screenshots/post-fix-retest-20260619-134345/`。
- `BUG-017`：发布成功后「查看市集页」打开 `/a/{slug}` 只显示通用占位。如果本期不交付公开能力页，请调整成功态入口/文案；如果 `marketUrl` 应可查看能力卡，则实现只读预览。
- `BUG-018`：STEP2 候选选择控件缺少清晰可见的选择 affordance 和 `aria-label`，用户容易停在“已识别候选但已选 0 项”。
- `BUG-009` 已由本轮空草稿 Step5 深链复测通过；不要为修 UI 重新引入“仅有 draftId 就伪造前序完成”的回归。
- 试用 runtime 当前按 contracts 是「按钮在 + 本期未开放」，不作为本轮修复 bug；但 Trial 未来打开时必须按 Figma 试用节点重新验收。

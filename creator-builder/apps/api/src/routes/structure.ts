...
export const STRUCTURE_ENDPOINTS: EndpointDecl[] = [
  // select 步（纯前端步的服务端落点：draft.selection）。
  {
    method: 'PATCH',
  ...
  // 读 manifest（创作者编辑态）：requireRole('creator') + handler owner 校验（40 §4.B，Codex#6）。
  ...
  // SSE structure 流：同源 Cookie 鉴权（建流前 HTTP 失败，脊柱 §11.C）。owner 校验在 handler 建流前做。
  {
    method: 'GET',

...
func newReporter() reporter {
	...
	return &ttyReporter{w: os.Stderr, color: os.Getenv("NO_COLOR") == ""}
}
...
// —— ANSI（品牌砖红 #A73718 / 橄榄绿 #4A6641 / 中性灰）——
const (
	ansiReset  = "\x1b[0m"
	ansiAccent = "\x1b[38;2;167;55;24m"
	ansiBold   = "\x1b[1;38;2;167;55;24m"
	ansiGreen  = "\x1b[1;38;2;74;102;65m"
	ansiDim    = "\x1b[38;2;150;145;138m"
...
func (p *plainReporter) success()                      { p.line("上传完成，回到网页查看云端解析进度。") }
...
func (t *ttyReporter) uploadProgress(done, total int) {
	...
	suffix := t.c(ansiDim, comma(done)+" / "+comma(total)+" 分片 · ") + t.c(ansiBold, itoa(pct)+"%")
	t.drawBar("上传", done, total, suffix, done == total)
}

func (t *ttyReporter) success() {
	t.endBar()
	fmt.Fprintln(t.w, t.c(ansiGreen, "  ✓ 上传完成")+t.c(ansiDim, "，回到网页查看云端解析进度。"))
}

...
第二批 `maintenance/retire-legacy-runtime.sql` 继续删除十二张旧运行、Builder 和模型审计表。
完成后保留十六张当前业务表、四张只读历史表和一张内部历史证据表；迁移账本仍是 `0021`。
历史资金和订单关联不被清空，运行上下文由不可变证据承接。两批步骤均须显式执行，不作用于共享库或 V2。

用户确认不再需要旧调用历史后，可独立执行第三批 `maintenance/retire-legacy-history.sql`，删除上述
四张历史表与内部证据表，剩余十六张当前业务表和迁移账本。账户余额、订单、资金流水与其中的历史
标识原值不变；钱包和充值校验不再查询旧表，旧调用明细仅在先前受控备份中保留。该步骤仍仅限 Test。

...
支持恢复的 SDK 使用独立 `createRecoverablePaymentClient`，V1 客户端保持原行为。初始 402 与三字段 Host 交接不变；Host 用当前登录身份通过 `/v2/payments` 创建和找回逻辑支付，并打开带 `version=2` 的平台收银台。`/v2/payments/{id}/recover` 只接受原尝试编号和提前持久化的恢复编号。创建或恢复响应不确定时查询原支付、保留原编号，不能用新编号补发。

V2 视图区分逻辑支付 `unpaid/completed/closed` 和渠道尝试状态；付款码过期不等于原业务已结束。只有平台已验签关单成功并查得原单 PAY_FAIL，才允许新的付款尝试和新渠道流水。SDK 和业务不能自行关单、改金额、换调用编号或判断扫码成功。恢复窗口为逻辑支付创建后 24 小时，最多三个总尝试；服务端决定是否可恢复，客户端不能从本地时间推断授权。

业务仍保存原输入、operationId/callId 和结果；Host 在真正入账后获取新身份断言并继续原业务。等待超时、用户离开、浏览器刷新和重新登录都保留原任务，未知结果不重新执行。新的 SDK 包应锁定精确来源和摘要；平台恢复开关开启且完成对应迁移/路由后才可以切换消费方。本实现与参考应用验证不代表 Mingli 已升级，也不代表已完成真人付款验收。

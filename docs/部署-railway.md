...
2. 导入历史,两种方式:
   - **⚡ 一键导入(Chrome/Edge)**:浏览器弹目录授权框 → 选主目录 → 「打开」→ 自动读 `~/.claude`+`~/.codex` → 进下一步。首次选一次目录,复访更轻。Safari/Firefox 回退「选文件夹」。
   - **🔌 连接本机(零选择,任何浏览器)**:点它 → 复制给出的一行命令 `curl -fsSL <BASE>/connect.mjs | node - <配对码>` → 在终端粘贴回车 → 本机助手(用其系统权限)读全部 Claude/Codex 历史并上传 → 网页轮询到后**自动进下一步**。**一个目录都不用选**;需本机已装 Node。
   详见 [`docs/研究-一键导入的技术边界与方案.md`](研究-一键导入的技术边界与方案.md)。

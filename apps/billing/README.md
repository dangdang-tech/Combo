...
- `src/recovery-repo.ts` 保存 V2 渠道付款尝试、恢复任务、关单与查单证据、收款事实和客户待处理款分录，原业务支付记录保持不变。
- `src/recovery-service.ts` 在当前用户的原支付下查询、关单和创建新付款尝试，恢复任务先落库再发渠道请求，进程接管只查询不重发。
- `src/recovery-routes.ts` 提供显式 V2 支付创建、查询、恢复与付款码接口；`src/recovery-page.ts` 展示恢复进度、原单核对和人工处理状态。
...
`BILLING_PAYMENT_RECOVERY_ENABLED=true` 才注册恢复接口，并要求独立 V2 0019 迁移完成。默认关闭；V1 的 token、requestKey、有效期与响应不改变。`/v2/payments` 保留原逻辑支付身份，每个支付在创建后 24 小时内最多有三个渠道付款尝试（包含首单）。初次创建仍使用标准 402 的短期凭证；之后恢复由当前登录会话授权，不重新使用过期的 V1 付款动作。

`POST /v2/payments/{paymentId}/recover` 接受 `recoveryKey` 和 `expectedAttemptId`，持久化后返回 202。相同恢复编号重放只返回当前状态，旧页面、跨用户和换载荷请求不能创建新单。服务端查原单、验签关单成功，再查到 PAY_FAIL，才保存新的渠道流水并发起一次预下单。查询暂时失败最多重试十五次；关单响应丢失或无法确认时转人工核对，不重发关单或预下单。后台查询每个尝试最多 120 次且间隔至少三十秒，多进程通过数据库领取查询与恢复任务。

`GET /v2/payments/{paymentId}` 区分逻辑支付与当前尝试；`GET /v2/payments/by-request-key/{requestKey}` 找回最初支付。`/payments/{paymentId}?version=2` 展示缺码、核对、恢复和人工处理状态，`/v2/payment-checkouts/{paymentId}` 只向当前登录用户返回付款码。恢复编号在发送前保存于页面会话存储。SDK 使用显式恢复客户端，旧客户端不会被改成恢复已关闭的付款动作。

同一渠道交易重复通知只入账一次。不同交易都成功时，首笔完成逻辑支付及原调用预留，其余收款记为 review_required，并追加平衡的渠道清算与客户待处理款分录，保留人工对账依据。它不会再次增加可消费钱包余额，也不会再次恢复业务。已有模型执行尝试与渠道付款尝试分开保存。

恢复事件分别记录预下单、保存预下单结果、查询、关单和入账阶段；故障原因只使用固定枚举，包括超时、传输失败、HTTP错误、JSON错误、签名错误、缺码、响应校验及存储错误。渠道原文、密钥、用户Cookie和付款码不进入事件或日志。签名通过的PAY_IN_PROCESS响应保留原交易号和可用二维码，没有二维码时保留待处理状态供恢复。

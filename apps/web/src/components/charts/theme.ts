...
// 配色基线：Figma XwOk3OdwHGSt6gviqS2Doy（冷纸底 + Combo 砖红），与 --cb-accent / --cb-heat-* 同源。
...
/** 次要文字 / 轴标签（与 --cb-muted 同源，冷灰绿）。 */
export const CHART_MUTED = '#66706c';
/** 主文字（与 --cb-fg 同源）。 */
export const CHART_FG = '#1c1b19';
/** 网格线 / 边框（与 --cb-border 同源，冷纸边界）。 */
export const CHART_BORDER = '#dbe0db';
/** 骨架/空槽底色（与 --cb-skeleton 同源）。 */
export const CHART_SKELETON = '#eef2ef';
/** 峰值标注强调色（砖红，对齐 Figma 峰值药丸）。 */
export const CHART_PEAK = '#a73718';

/**
 * 热力图五档色阶（level 0→4，浅→深；砖红渐进，与 --cb-heat-* 同源）。
 * level 0 = 无活跃（冷纸底），level 4 = 最活跃（砖红）。
 */

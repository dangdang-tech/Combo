...
const STEP_LABELS = ['导入工作历史', '识别 Agent', '生成页面', '试用与发布'] as const;
...
export function deriveCreationJourney(snapshot: CreationJourneySnapshot): JourneyStep[] {
  ...
  const hasVerifiedTrial = Boolean(snapshot.hasTrialResult);
  const hasTrialResult = Boolean(hasVerifiedTrial || snapshot.batchId || hasPublishedResult);

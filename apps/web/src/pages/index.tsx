...
import { isCloudReviewEnvironment, loginUrl } from '../shell/auth.js';
    ...
    <section className="cb-page cb-public" aria-labelledby="cb-notfound-title">
      <div className="cb-public__notice">
        ...
        <div className="cb-public__actions">
          ...
          <a href={loginUrl()} className="cb-public__action cb-public__action--ghost">
            {isCloudReview ? '进入云端评审' : '去登录'}
          </a>
        </div>
      </div>
    </section>
    ...
    <section className="cb-page cb-public" aria-labelledby="cb-login-title">
      <div className="cb-public__notice">
        ...
        <p className="cb-public__lead">
          {isCloudReview
            ? failed
              ? '这次预览身份没有建立成功，请重新进入。如果多次失败，可凭下方反馈代码联系我们。'
              : '预览账号和数据与主环境隔离，进入后会回到刚才的评审页面。'
            : failed

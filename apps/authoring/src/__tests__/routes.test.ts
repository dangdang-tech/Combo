...
describe('route registry self-check', () => {
  it('registers the full contract endpoint set (56 in-scope callable endpoints)', () => {
    ...
    //   + 公开创作者主页 slug 读端点 1（GET /creators/by-slug/:slug/profile，/c/:slug 真数据源）= 56 个本期可调用端点
    // （§2.9 的 3 个消费链路读端点本期范围外、仅冻结、不计入）。
    expect(ALL_ENDPOINTS).toHaveLength(56);
  });

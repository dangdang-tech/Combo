...
class FakePairDb {
  ...
  async query<R = Record<string, unknown>>(
    ...
    // —— readPairingStatus：SELECT owner_user_id, draft_id, phase, job_id, uploaded_parts, total_parts, (expires_at<=now) ——
    if (
...
describe('connectPairStatusHandler（owner 校验 + phase 折算）', () => {
  it('本人且配对挂接 draft → 200 返回 draftId，供页面恢复正确草稿', async () => {
    const db = new FakePairDb();
    const p = db.seedPairing({ owner_user_id: 'creator-1', draft_id: 'draft-7' });
    ...
    expect(sent.code).toBe(200);
    const data = (sent.body as { data: Record<string, unknown> }).data;
    expect(data.draftId).toBe('draft-7');
  });
  ...
  it('非本人 → 404（不暴露存在性，10-auth §6.3）', async () => {
    const db = new FakePairDb();
    const p = db.seedPairing({ owner_user_id: 'creator-1', draft_id: 'draft-secret' });

...
//   - missingPublishFields：发布前必填软字段校验（name/tagline 非空 + 封面来源合法，§1.2 步1）。
//     缺则路由层据此出 422 PUBLISH_MISSING_FIELDS（details.missingFields，发布-24）。绝不裸转圈/裸错误码。
import { createHash } from 'node:crypto';
import { canonicalManifest, type Manifest, type CoverInput } from '@cb/shared';
...
/** 发布入参（封面）—— 用于必填校验（manifest 之外的发布期入参，§1.2 步1）。 */

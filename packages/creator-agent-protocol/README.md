...
它只接收有界 JSON 整理结果。可选 `client` 只接受 `codex` 或 `claude`，后者对应 `claude_available_context`；
省略或指定 `codex` 保留原有 `codex_available_context` Draft 与 Package 字节。两者都固定
`not_verified / partial_or_unknown`，不接收完整对话、来源选择器或调用方认证声明。客户端名不是身份验证。
`client` 不进入 Draft 的独立字段，Draft fingerprint 绑定规范的 request、content 与相应来源，

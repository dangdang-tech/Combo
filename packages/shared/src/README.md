...
- `domains/` 定义认证和普通充值订单的运行时校验规则与类型。
- `__tests__/auth.test.ts` 检查邮箱请求、六位验证码、回跳白名单、会话 Cookie、成功包络、安全错误和就绪依赖。
- `__tests__/pending-recovery.test.ts` 检查普通充值订单的严格金额与视图契约。
...
authoring 使用 core 的错误、健康和 trace 工具，constants 的路由前缀，以及 domains 中的认证与普通充值契约。web 使用认证视图、认证请求与结果、站内导航规则和发布身份。具体文件关系记录在各子目录 README 中。

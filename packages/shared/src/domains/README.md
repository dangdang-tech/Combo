...
- `capability.ts` 定义能力项域：库内轻量索引视图 `CapabilityView`、存在 MinIO 里的完整可运行定义 `CapabilityDefinition`（提取流水线写入、试用端读出注入 agent，是两个服务之间唯一的契约缝），以及发布动作的结果。
- `trial.ts` 定义试用域：会话、消息、产物的视图和建会话、发消息的请求体；消息内容是 agent 原生分块格式，共享层只约束到「是数组」，严格校验在 runtime 侧。

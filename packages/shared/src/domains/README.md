...
这个目录按业务域定义对外接口的数据形状与校验规则，目前只保留认证和普通充值订单。每个域同时导出 Zod 运行时 schema 和推导出的 TypeScript 类型。
...
- `pending-recovery.ts` 定义普通充值订单的严格输入与订单视图，并拒绝旧恢复编号。金额以规范十进制分字符串表示，并限制在 PostgreSQL bigint 正数范围内。
- `index.ts` 汇总转出以上全部文件。
...
authoring 的账号模块使用邮箱请求、验证结果、当前用户、登出、Cookie 和回跳契约，billing 模块使用普通充值订单视图。web 使用相同的认证请求、响应和回跳定义实现自定义登录与站内导航。
